...
// GenerateToken create new token with HS256 signing method
...
// GetEmailFromToken parse the given auth token and return user email in claims if recieved token is valid otherwise return empty string
...
// Skipper return true if this path not require jwt auth. otherwise false
...
// SecretToBytes decode jwt secret key that encoded with base64 to byte array.
// Return non nil err if string not encoded with hex or length not equal to 64
